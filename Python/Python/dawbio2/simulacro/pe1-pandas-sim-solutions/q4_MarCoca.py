...
def get_tuberculosis_deaths(entries: pd.DataFrame) -> pd.DataFrame:
    ...
    diseases_mask = get_state_disease_mask('TUBERCULOSIS',['NY',"CA","IL"])
    ...
    diseases = pd.pivot_table(diseases,index="week",columns="state",values="num_deaths", fill_value=0).reset_index().reindex(columns=sorted_columns)
