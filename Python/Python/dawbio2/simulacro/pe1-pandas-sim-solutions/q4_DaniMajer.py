...
def get_tuberculosis_deaths(entries: pd.DataFrame) -> pd.DataFrame:
    ...
    deaths_CA: pd.DataFrame = ((entries.loc[tuberculosis_CA_mask]).groupby("week", as_index=False).agg(CA_deaths=("num_deaths", "sum")))
    ...
    # =======================================================
    df_merged: pd.DataFrame = reduce(lambda left, right: pd.merge(left, right, on=["week"], how="outer"),[deaths_NY, deaths_CA, deaths_IL],)
